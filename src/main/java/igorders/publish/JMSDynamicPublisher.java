...
@Component
public class JMSDynamicPublisher {
    ...
    public void publishMessage(String tcpServerURL, String username, String password, boolean isQueue,
                               String nameOfDestination, List<Order> orders) {
        ...
        try {
            ...
            //TODO add multi threading for publishing
    ...
    private void createQueueTopic() {
        /*        boolean transacted = false;
        TopicConnection connection = conFactory.createConnection();
        try {
            connection.start();
        } catch (JMSException e) {
            e.printStackTrace();
        ...
        try {
            TopicSession session = connection.createTopicSession(transacted, Session.AUTO_ACKNOWLEDGE);
            Topic myTopic = session.createTopic("TOPIC1");
        } catch (JMSException e) {
            e.printStackTrace();
        }
*/
